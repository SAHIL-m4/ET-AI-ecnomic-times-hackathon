# MeetMind — Autonomous Meeting Intelligence Agent

> ET AI Hackathon 2026 · Problem Statement 2: Agentic AI for Autonomous Enterprise Workflows

MeetMind deploys a four-agent AI pipeline that takes a meeting transcript and autonomously extracts every decision, creates structured tasks, assigns owners, detects SLA risks — and produces a full auditable trail of every reasoning step. Humans stay in control at two approval gates; everything else runs itself.
...
Four specialised agents. Two human approval gates. One self-correction loop. Zero manual follow-up.

---

## Demo

Upload `demo/sample_transcript.txt` and watch:
1. Agent 1 diarises speakers and segments topics
2. Gate 1 — you approve the parsed context (30 seconds)
3. Agent 2 extracts 6 decisions with confidence scores
4. Agent 4 (Watchdog) flags 2 ambiguous owners → triggers self-correction
5. Agent 2 re-runs, resolves 1 ambiguity via earlier speaker context
6. Agent 3 creates 5 structured tasks with priorities
7. Gate 2 — you review before pushing to board
8. Kanban board populates. Full audit trail is live and explorable.
...
| Metric | Saving |
|---|---|
| Time saved per employee per week | 3.5 hours |
| Annual cost saving (₹320/hr) | ₹5.2 Cr |
| SLA breach prevention | ₹60L/yr |
| **Total estimated ROI** | **₹5.8 Cr/yr** |

See `impact_model.md` for full assumptions and math.

---

## Team

Built at ET AI Hackathon 2026 in 24 hours.

---

## Tech Stack

- **Agents**: Python + Anthropic Claude API (`claude-sonnet-4-6`)
- **Backend server**: FastAPI + Server-Sent Events (SSE)
- **Frontend**: React 18 + Tailwind CSS
- **State**: In-memory + JSON audit log
